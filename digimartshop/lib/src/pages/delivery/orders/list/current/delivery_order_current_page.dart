...
import 'package:digimartbox/src/pages/delivery/orders/list/current/delivery_order_current_controller.dart';
import 'package:digimartbox/src/widgets/no_data_widget.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class DeliveryOrderCurrentPage extends StatelessWidget {
  DeliveryOrderCurrentPage({Key? key}) : super(key: key);
  DeliveryOrderCurrentController deliveryController = Get.put(DeliveryOrderCurrentController());
        ...
        child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              if (deliveryController.isLoading.value) {
                ...
                return NoDataWidget(textMessage: 'No hay ordenes actuales.');
              }
            }
        ),
      );
                ...
                Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                          color: deliveryController.getColorByStatus(order.status),
  ...
  _buttonOrder(OrderListDelivery order) {
    return Column(
      children: <Widget>[
        Container(
          padding: const EdgeInsets.symmetric(vertical: 5),
            child: Divider(height: 2, color: Colors.grey[800], indent: 8, endIndent: 8)
        ),
        SizedBox(
          width: double.infinity,
          child: ElevatedButton(
            onPressed: () => deliveryController.executeButtonOrder(order),
            child: Text(
              order.buttonText?.toUpperCase() ?? '',
